(function ($) {
  ...
  function showToast(message, type) {
    ...
    // Enforce max stack size (e.g., 4)
    var max = 4;
    var existing = stack.children('.ai1wm-toast-item');
    ...
    var close = $('<button type="button" class="ai1wm-toast-close" aria-label="Dismiss">\u00d7</button>');
    var text = $('<div class="ai1wm-toast-text"></div>').text(message);
    item.append(close).append(text);
    stack.append(item);
    setTimeout(function(){ item.fadeOut(200, function(){ item.remove(); }); }, 3600);
  }

  // Dismiss toast manually
  $(document).on('click', '.ai1wm-toast-close', function(){
    var item = $(this).closest('.ai1wm-toast-item');
    item.stop(true,true).fadeOut(150, function(){ item.remove(); });
  });
  ...
  function ensureUploadProgress(statusElement) {
    ...
    if (!prog.length) {
      ...
      var percent = $('<div class="ai1wm-upload-percent"></div>');
      prog.append(bar).append(label).append(percent);
    ...
    var iv = setInterval(function(){
      $.ajax({ url: url, type: 'GET', dataType: 'json', data: { archive: archive } })
        .done(function(res){
          ...
          // Progress if bytes available
          var wrap = statusElement.closest('.ai1wm-backup-actions').find('.ai1wm-upload-progress');
          var bar = wrap.find('.ai1wm-progress');
          var barInner = wrap.find('.ai1wm-progress__bar');
          var pctEl = wrap.find('.ai1wm-upload-percent');
          var total = parseInt(st.bytes_total || 0, 10);
          var done = parseInt(st.bytes_done || 0, 10);
          if (total > 0) {
