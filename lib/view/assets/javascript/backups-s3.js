(function ($) {
  ...
  function showToast(message, type) {
    var el = $("#ai1wm-toast");
    if (!el.length || !message) return;
    el.removeClass("ai1wm-toast--success ai1wm-toast--error ai1wm-toast--info");
    if (type === 'error') el.addClass('ai1wm-toast--error');
    else if (type === 'success') el.addClass('ai1wm-toast--success');
    else el.addClass('ai1wm-toast--info');
    el.text(message);
    el.addClass('ai1wm-toast--show');
    if (toastTimer) clearTimeout(toastTimer);
    toastTimer = setTimeout(function(){ el.removeClass('ai1wm-toast--show'); }, 3200);
  }
