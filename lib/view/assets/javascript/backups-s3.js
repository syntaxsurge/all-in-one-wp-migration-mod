...
function parseLogData(element) {
    // Try to read pre-baked JSON first.
...
$(document).on('show.bs.modal', '#ai1wmS3LogModal', function (e) {
    ...
    if (!trigger || !trigger.length) {
        body.text(ai1wm_s3.strings.no_log);
        titleEl.text(ai1wm_s3.strings.modal_title);
        return;
    }
    ...
    if (type === 'upload') {
        ...
        titleEl.text(ai1wm_s3.strings.modal_title_with_name.replace('%s', filename));
        body.text(ai1wm_s3.strings.preparing);

        // Kick off server-side job as before
        ...
        if (!ai1wm_s3.configured) {
            body.text(ai1wm_s3.strings.saving_required);
            return;
        }
        ...
        $.ajax({
            ...
            if (response && response.success && response.data && response.data.status) {
                ...
                body.text(ai1wm_s3.strings.failed_prefix + ' ' + msg);
            } else {
                var generic = ai1wm_s3.strings.generic_error;
            ...
            body.text(ai1wm_s3.strings.failed_prefix + ' ' + message);
        });
    }
});
