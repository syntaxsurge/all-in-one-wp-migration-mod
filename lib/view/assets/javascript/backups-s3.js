(function ($) {
	...
	var DEFAULT_STRINGS = {
		...
		copy_blocked_active: 'An upload is already %s for this backup. Please wait for it to finish before copying again.',
		confirm_replace: 'A remote copy already exists (%s). Replace it? This will delete the existing remote backup before uploading again.',
		replace_cancelled: 'Upload cancelled.',
		replacing: 'Preparing to replace the existing remote backup...',
		state_labels: {
			queued: 'Queued',
...
function showStatus(statusElement, message, state) {
    ...
    statusElement.text('');
    if (!statusElement.hasClass('ai1wm-hide')) {
        statusElement.addClass('ai1wm-hide');
    }
}
...
$(document).on('show.bs.modal', MODAL_SELECTOR, function (e) {
	...
	if (type === 'upload') {
		...
		var currentState = (existingStatus.state || trigger.data('state') || '').toString().toLowerCase();
		var busyStates = { queued: true, in_progress: true, pending: true };
		if (busyStates[currentState]) {
			var stateLabel = formatState(currentState) || currentState || strings.state_labels.pending || '';
		...
		if (currentState === 'success') {
			...
			if (!window.confirm(strings.confirm_replace.replace('%s', remoteName))) {
