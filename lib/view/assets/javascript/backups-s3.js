(function ($) {
  ...
  $(document).on("click", "#ai1wm-s3-list tbody tr[data-type=object] .ai1wm-s3-copy", function () {
    ...
    $.ajax({
      ...
      .done(function (res) {
        if (res && res.success && res.data && res.data.backup) {
          var name = res.data.backup && res.data.backup.filename ? res.data.backup.filename : key;
          var msg = (strings.copy_success || "Downloaded: %s").replace("%s", name);
          showBrowseFeedback(msg, false);
        } else if (res && res.data && res.data.errors) {
