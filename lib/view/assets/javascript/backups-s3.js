(function ($) {
  ...
  function showBrowseFeedback(msg, isError) {
    ...
    el.removeClass("ai1wm-error ai1wm-success");
    if (msg) {
      el.text(msg);
      el.addClass(isError ? "ai1wm-error" : "ai1wm-success");
    } else {
      el.text("");
    }
  }
    ...
    $.ajax({ url: listUrl, type: "GET", dataType: "json", data: params })
      .done(function (res) {
        if (res && res.success && res.data && res.data.result) {
          ...
          showBrowseFeedback(strings.listing_error || "Listing failed.", true);
        } else {
          showBrowseFeedback(strings.listing_error || "Listing failed.", true);
        }
      })
  ...
  $(document).on("click", "#ai1wm-s3-list tbody tr[data-type=object] .ai1wm-s3-copy", function () {
    ...
    showBrowseFeedback(strings.copy_started || "Download started...", false);
    ...
    $.ajax({
      ...
      .done(function (res) {
        if (res && res.success && res.data && res.data.job && res.data.job.state) {
          ...
          showBrowseFeedback(msg2, true);
          setButtonBusy(btn, false);
        } else {
          showBrowseFeedback(strings.copy_failed || "Download failed.", true);
          setButtonBusy(btn, false);
        }
      })
      .fail(function (jqXHR) {
  ...
  $(document).on("click", "#ai1wm-s3-list tbody tr[data-type=object] .ai1wm-s3-cancel", function () {
    ...
    $.ajax({
      ...
      .done(function (res) {
        if (res && res.success) {
          showBrowseFeedback(strings.cancelled || "Cancelled.", false);
        } else if (res && res.data && res.data.errors) {
          var msg = res.data.errors.join("\n");
          showBrowseFeedback(msg, true);
        } else {
          showBrowseFeedback(strings.copy_failed || "Operation failed.", true);
        }
      })
      .always(function () {
        setButtonBusy(btn, false);
        ...
        .done(function (res) {
          if (res && res.success && res.data && res.data.job) {
            ...
            if (job.state === 'in_progress') {
              showBrowseFeedback(formatProgress(job.bytes_done || 0, job.bytes_total || 0), false);
              row.find('.ai1wm-s3-prog').text(formatProgress(job.bytes_done || 0, job.bytes_total || 0));
              row.find('.ai1wm-s3-state').text(formatState(job.state) || job.state);
            } else if (job.state === 'success') {
              var name = job.filename || key;
              showBrowseFeedback((strings.copy_success || "Downloaded: %s").replace("%s", name), false);
              stopPollingDownload(key);
              if (strings.refreshing) { showBrowseFeedback(strings.refreshing, false); }
              refreshBackupsTablePartial();
              row.remove();
            } else if (job.state === 'failed') {
              showBrowseFeedback((strings.copy_failed || "Download failed: %s").replace("%s", job.message || ""), true);
              stopPollingDownload(key);
              btn.text(strings.copy_here || "Copy to Backups").removeClass("ai1wm-s3-cancel").addClass("ai1wm-s3-copy");
              row.find('.ai1wm-s3-state').text(formatState(job.state) || job.state);
              row.find('.ai1wm-s3-prog').text('—');
  ...
  function renderActiveDownloads(initial) {
    ...
    keys.forEach(function(k){
      ...
      row.append('<td class="ai1wm-s3-prog">'+ prog +'</td>');
  ...
  function ensureDownloadRow(key, filename) {
    ...
    if (!row.length) {
      ...
      row.append('<td class="ai1wm-s3-prog">'+ formatProgress(0,0) +'</td>');
